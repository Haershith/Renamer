    ...
    if Config.FORCE_SUB:
        try:
            ...
            if "belongs to a user" in str(e):
                ...
                if m.from_user.id in Config.AUTH_USERS:
                    return await m.reply_text("Forcesub username must be a channel username Not yours or any other users username @MutyalaHarshith")
            logger.error(e)
            return await m.reply_text("Some thing went wrong. Try again and if same issue occur contact [our group](https://t.me/Harshith_BoTs)", disable_web_page_preview=True, quote=True)
